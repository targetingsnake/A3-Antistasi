...
Example:
    [cursorObject, true] call A3A_fnc_thermobaricDamage;  // Burn whatever you are looking at.
*/

private _filename = "functions\bombs\fn_thermobaricDamage.sqf";

params [
    ["_victim",objNull,[objNull]],
	["_victimFar",objNull,[objNull]],
...
if (isNull _victimFar) exitWith {false};  // Silent, likely for script to find some null objects somehow.

if (isNil {
    if (!alive _victimFar || {!isDamageAllowed _victimFar} || {isObjectHidden _victimFar}) exitWith {nil};   // Hidden objects could be Zeus or other important mission things.
    1;
}) exitWith {true};
...
private _fnc_init = 'params ["_victimFar"];';                  // params ["_victim"] No return required
private _fnc_onTick = 'params ["_victimFar","_tickCount"];';   // params ["_victim","_tickCount"] No return required
private _fnc_final = 'params ["_victimFar"];';                 // params ["_victim"] No return required

private _invalidVictimFar = false;
private _invalidVictim = false;

switch (true) do {
	// ===== damage for _victim aka within the radius =====
	case (_victim isKindOf "CAManBase") : {'_victim' setDamage 1;';';};
    case (_victim isKindOf "Man"): {'_victim' setDamage 1;';';};  // Goats, Sneks, butterflies, Rabbits can be blessed by Petros himself.
    case (_victim isKindOf "AllVehicles"): {'_victim' setDamage 1;';';};
	...
	case (_victimFar isKindOf "CAManBase"): {  // Man includes everything biological, even animals such as goats ect...
        if (A3A_hasACEMedical) then {
            ...
            'if (alive _victimFar) then {
                [ _victimFar, 1*' + str _damagePerTick + ' , "Body", "grenade"] call ace_medical_fnc_addDamageToUnit;'+  // Multiplier might need to be raised for ACE.
            '};

            if (alive _victimFar && {!(_victimFar getVariable ["ACE_isUnconscious",false])} && {(('+ str _timeBetweenTicks + '*_tickCount) mod 2) isEqualTo 0}) then {'+  // Once every 2 seconds
            '    playSound3D [selectRandom A3A_sounds_soundInjured_max,vehicle _victimFar, false, getPosASL vehicle _victimFar, 1.75, 1, 200];'+  // For `vehicle _victim` see https://community.bistudio.com/wiki/playSound3D Comment Posted on November 8, 2014 - 21:48 (UTC) By Killzone kid
            '};';
        } else {
            _fnc_onTick = _fnc_onTick +
            'if (alive _victimFar) then {
                _victimFar setDamage [(damage _victimFar + ' + str _damagePerTick + ') min 1, true];'+
            '};

            if (alive _victimFar && {(('+ str _timeBetweenTicks + '* _tickCount) mod 2) isEqualTo 0}) then {'+  // Once every 2 seconds
            '    playSound3D [selectRandom A3A_sounds_soundInjured_max,vehicle _victimFar, false, getPosASL vehicle _victimFar, 1.75, 1, 200];'+  // For `vehicle _victim` see https://community.bistudio.com/wiki/playSound3D Comment Posted on November 8, 2014 - 21:48 (UTC) By Killzone kid
            '};';
        };
        if (_particles) then {
    ...
    case (_victimFar isKindOf "Man"): {_invalidVictimFar = true;};  // Goats, Sneks, butterflies, Rabbits can be blessed by Petros himself.
    case (_victimFar isKindOf "AllVehicles"): {
        // Vehicles should be damaged as much as possible but salvageable. This would give thermobaric a unique tactic of clearing AI from vehicles allowing them to be repaired, refuelled and requestioned.
        _fnc_init = _fnc_init +
        ...
        if !(getAllHitPointsDamage _victimFar isEqualTo []) then { // The static has a class entry, but its empty and getAllHitPointsDamage will return empty array
            _fnc_onTick = _fnc_onTick +
                '_victimFar setHitPointDamage ["HitHull",(((_victimFar getHitPointDamage "HitHull") + ' + str _damagePerTick + ') min 0.8) max (_victimFar getHitPointDamage "HitHull")];'+ // Limited to avoid vehicle being destroyed. Will not decrease vehicle damage if it was initially above 80%
                '{
                    _victimFar setHitPointDamage [_x,((_victimFar getHitPointDamage _x) + ' + str _damagePerTick + ') min 1];
                } forEach ' + str ((getAllHitPointsDamage _victim)#0 - ["hithull"]) + ';';
        } else {
            _fnc_onTick = _fnc_onTick + '_victimFar setDamage (((damage _victimFar + ' + str _damagePerTick + ') min 0.8) max damage _victimFar);'; // Limited to avoid vehicle being destroyed. Will not decrease vehicle damage if it was initially above 80%
        };

        _fnc_onTick = _fnc_onTick +
            'private _thermalHeat = 0.75*(_tickCount/'+ str _totalTicks +') + 0.25;'+  // The vehicles shouldn't snap to cold when the thermobaric effect starts begin.
    ...
    case (_victimFar isKindOf "GroundWeaponHolder"): {  // Is a building, therefore needs to be above buildings
        _totalTicks = 1;
