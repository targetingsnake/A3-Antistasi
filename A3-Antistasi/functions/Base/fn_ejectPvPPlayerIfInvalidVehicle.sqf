...
if (_veh != lastVehicleSpawned) then
{
	...
	if (!((typeOf _veh) in (vehNATOPVP + FactionGet(inv,"pvpVehicles") + [CSATMG] + [staticATInvaders] + [staticAAInvaders] + [NATOMG] + [staticATOccupants] + [staticAAOccupants])) && !(_role == "Cargo")
