...
_sideX = side player;
_nameX = if (_sideX == Occupants) then {FactionGet(occ,"name")} else {FactionGet(inv,"name")};
...
_typeBike = if (_sideX == Occupants) then {selectRandom FactionGet(occ,"pvpVehicles")} else {selectRandom FactionGet(inv,"pvpVehicles")};
