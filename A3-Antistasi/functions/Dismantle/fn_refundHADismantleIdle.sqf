...
if (_activeWatts != 0) then {
    ...
    _actionData set ["_completionProgress",_joules - _joulesRemaining];
    _actionData set ["_completionGoal",_joules];
    ...
    _topText = format [A3A_richAction_pressSpaceTo,"color='#ffae00'",_actionText];
} else {
    _topText = format [A3A_richAction_pressSpaceTo,"color='#ffae00'","Dismantle"];
